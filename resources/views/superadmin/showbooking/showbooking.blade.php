    ...
    <div class="search-set">
        ...
        <div class="mb-4 mt-4 ms-3">
            <div class="d-flex flex-wrap gap-2 align-items-center">
                <a href="{{ route('superadmin.showbooking.showBooking', array_filter(['search' => request('search'), 'month' => request('month'), 'year' => request('year'), 'marketing' => request('marketing')], fn($v) => filled($v))) }}"
                   class="btn btn-sm {{ !$department ? 'btn-primary' : 'btn-outline-primary' }}">
                    All
                </a>

                @foreach($departments as $dept)
                    <a href="{{ route('superadmin.showbooking.showBooking', array_filter(['department' => $dept->id, 'search' => request('search'), 'month' => request('month'), 'year' => request('year'), 'marketing' => request('marketing')], fn($v) => filled($v))) }}"
