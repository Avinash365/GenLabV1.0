            ...
            <div class="card shadow-sm h-100">
                <div class="card-body p-4">
                    ...
                    <form id="gstinForm" class="row g-2 align-items-end" method="POST">
                        ...
                        <div class="col-sm-8">
                            <label class="form-label">Enter GSTIN</label>
                            <input type="text" name="gstin" id="gstinInput" class="form-control" placeholder="Enter GSTIN"
                                required>
                        </div>
                        <div class="col-sm-4">
            ...
            <div class="card shadow-sm h-100 border-0">
                <div class="card-body p-4">
                    ...
                    <form id="gstinUploadForm" enctype="multipart/form-data" method="POST"
                        ...
                        <!-- Upload Area -->
                        <label for="gstinFile" class="w-100 border border-dashed rounded-3 p-4 text-center bg-light mb-3"
            ...
            <div class="page-header d-flex justify-content-between align-items-center">
                ...
                <div class="page-btn">
                    <button type="submit" class="btn btn-danger"
                        formaction="{{ route('superadmin.invoices.generateInvoice', $invoice->id) }}">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <table class="table table-bordered mb-4">
                        <tr>
                            <th>Client Name</th>
                            <td class="noteditable" id="td_client_name">{{ $invoice->relatedBooking->client_name ?? 'N/A' }}
                            </td>
                            <th>Marketing Person</th>
                            <td class="noteditable" id="td_marketing_person">
                                {{ $invoice->relatedBooking->marketingPerson->name ?? 'N/A' }}</td>
                        </tr>
                        <tr>
                            <th>Invoice No </th>
                            <td class="noteditable" id="td_invoice_no">{{$invoice->invoice_no ?? ''}}</td>
                            <th>Reference No</th>
                            <td contenteditable="true" class="editable" id="td_reference_no">
                                {{ $invoice->relatedBooking->reference_no ?? ''}}</td>
                        </tr>
                        <tr>
                            <th>Invoice Date</th>
                            <td contenteditable="true" class="editable" id="td_invoice_date">{{ $invoice->invoice_date }}
                            </td>
                            <th>Letter Date</th>
                            <td contenteditable="true" class="editable" id="td_letter_date">
                                {{ \Carbon\Carbon::parse($invoice->letter_date)->format('d-m-Y') ?? '' }}</td>
                        </tr>
                        <tr>
                            <th>Name of Work</th>
                            <td contenteditable="true" class="editable" id="td_name_of_work">
                                {{ $invoice->name_of_work ?? '' }}</td>
                            <th>Bill Issue To</th>
                            <td contenteditable="true" class="editable" id="td_bill_issue_to">{{ $invoice->issue_to ?? '' }}
                            </td>
                        </tr>
                        <tr>
                            <th>Client GSTIN</th>
                            <td contenteditable="true" class="editable" id="td_client_gstin">
                                {{ $invoice->client_gstin ?? '' }}</td>
                            <th>Address</th>
                    ...
                    <table class="table table-bordered mb-4" id="invoiceTable">
                        ...
                        <tfoot>
                            ...
                            <tr>
                                ...
                                <td contenteditable="true" class="editable" id="cgstPercent">{{$invoice->cgst_percent ?? 0}}
                                </td>
                                <th id="cgstAmount">0.00</th>
                            </tr>
                            <tr>
                                <th colspan="4" class="text-end">SGST %</th>
                                <td contenteditable="true" class="editable" id="sgstPercent">{{$invoice->sgst_percent ?? 0}}
                                </td>
                                <th id="sgstAmount">0.00</th>
                            </tr>
                            <tr>
                                <th colspan="4" class="text-end">IGST %</th>
                                <td contenteditable="true" class="editable" id="igstPercent">{{$invoice->igst_percent ?? 0}}
                                </td>
                    ...
                    <table class="table table-bordered mb-4">
                        <tr>
                            <th>Instructions</th>
                            <td class="noteditable" id="td_bank_instructions">
                                {{ $bankInfo->instructions ?? 'ABCSVHGVGHVSVGHSVD' }}</td>
                        </tr>
                        <tr>
                            <th>Bank Name</th>
                    ...
                    <div class="d-flex justify-content-end align-items-center gap-3 mb-3">
                        ...
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="typeOption" id="typeInvoice"
                                value="tax_invoice" {{ $invoice->type === 'tax_invoice' ? 'checked' : '' }}>
                            <label class="form-check-label" for="typeInvoice">Tax Invoice</label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="typeOption" id="typePI"
                                value="proforma_invoice" {{ $invoice->type === 'proforma_invoice' ? 'checked' : '' }}>
                    ...
                    <div class="d-flex justify-content-end">
                        <button type="submit" class="btn btn-success"
                            formaction="{{ route('superadmin.invoices.update', $invoice->id) }}">
