    ...
    <div class="page-header">
        ...
        <ul class="table-top-head list-inline d-flex gap-3">
            <li class="list-inline-item">
                ...
                @endphp
                <a href="{{ route('superadmin.accounts.export.pdf') }}{{ $exportQ ? ('?'.$exportQ) : '' }}" class="no-loader export-link export-pdf" data-bs-toggle="tooltip" title="PDF" data-total="{{ $totalItems }}" data-has-filters="{{ count($exportParams) ? 1 : 0 }}" data-limit="{{ $exportLimit }}"><div class="fa fa-file-pdf"></div></a>
            </li>
            <li class="list-inline-item">
                @php
                    // Use same export params (no pagination) for Excel
                @endphp
                <a href="{{ route('superadmin.accounts.export.excel') }}{{ $exportQ ? ('?'.$exportQ) : '' }}" class="no-loader export-link export-excel" data-bs-toggle="tooltip" title="Excel" data-total="{{ $totalItems }}" data-has-filters="{{ count($exportParams) ? 1 : 0 }}" data-limit="{{ $exportLimit }}">
    ...
    <div class="card">
        ...
        <div class="card-header d-flex flex-wrap align-items-center justify-content-between gap-2">
            ...
            <div class="search-set">
                <form method="GET" action="{{ route('superadmin.accounts.export') }}" class="d-flex input-group">
                        {{-- Preserve month & year --}}
            ...
            <div class="search-set">
                <form method="GET" action="{{ route('superadmin.accounts.export') }}" class="d-flex align-items-center gap-2">
                     {{-- Preserve search --}}
        ...
        <div class="mb-4 mt-4 ms-3">
            <div class="d-flex flex-wrap gap-2 align-items-center">
                ...
                @if(isset($marketingPersons) && $marketingPersons->count())
                    <form method="GET" action="{{ route('superadmin.accounts.export') }}" class="ms-auto me-3 d-flex align-items-center">
                ...
                <table class="table">
                    ...
                    <tbody>
                        ...
                        <tr class="table-row">
                            ...
                            <td class="truncate-cell amount-cell">
                                <div class="cell-inner text-end" data-bs-toggle="tooltip" title="{{ isset($item->amount) ? number_format($item->amount,2) : '-' }}">
                            ...
                            <td class="payment-cell">
                                <div class="cell-inner" data-bs-toggle="tooltip" title="{{ $item->booking?->payment_option ?? '-' }}">
                                    @php
                                        $po = $item->booking?->payment_option ?? null;
                                        $poLabel = '-';
                                        if ($po === 'bill') {
                                            $billNo = null;
                                            if (!empty($item->booking) && !empty($item->booking->generatedInvoice) && !empty($item->booking->generatedInvoice->invoice_no)) {
                                                $billNo = $item->booking->generatedInvoice->invoice_no;
                                            } elseif (!empty($item->booking) && !empty($item->booking->bill_no)) {
                                                $billNo = $item->booking->bill_no;
                                            }
...
<style>
    ...
    th.amount-col, td.amount-cell { text-align: right; }
    td.amount-cell .cell-inner { display:block; text-align:right; }
